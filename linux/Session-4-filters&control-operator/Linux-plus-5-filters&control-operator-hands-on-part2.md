...
```bash
sed 's/linux/ubuntu/' sed.txt # linux ü ubuntu ile degistiririz set.txt de. bu sekilde sadece ilk kelimeyi degistirir
`sed 's/linux/ubuntu/2' sed.txt #2.yi degistirir her satirda
sed 's/linux/ubuntu/2i' sed.txt #burda buyuk kucuk harf e bakmadan tüm satirlara uyguladi 
...
```bash
sed 's/linux/ubuntu/ig' sed.txt # global tüm satirdaki buyuk kucuk linuxleri degistirir
```
...
```bash
sed 's/linux/ubuntu/2ig' sed.txt # her satirda 2. den sonraki tüm linux leri degistirir 
```
...
```bash
sed '2 s/linux/ubuntu/ig' sed.txt #2.satirdan itibaren hepsini
```
...
```bash
awk '{print}' awk.txt #burda tek tirnak kullanmak önemli cift olunca bazen sorun olabilio. istenilen dosyayi print et demis oluyoruz
```
...
```bash
awk '/This/ {print}' awk.txt #awk dosyasinda This i iceren satirlari getir
```
...
```bash
awk '{print $2,$4}' awk.txt # 2. ve 4. sütunu getirir,önce 2 yi sonra 4 ü getirir,cut ta sira önemli degildi burda önemli
```
...
```bash
awk -F: '{print $2}' awk.txt #ayraci burda F le gösterioz, : dan ayir ve 2. sütunu getir
```
...
```bash
awk '{ if($7 == "3") print $0;}' awk.txt #eger 7.sütunun 3 e esitse o satirin tamamini print et (print $0) bu tamami demek
```
...
## Part 3 - crontab
# crontab.guru adresine bak bu konu icin
