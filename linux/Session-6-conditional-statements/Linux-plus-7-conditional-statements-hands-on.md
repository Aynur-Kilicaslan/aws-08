...
```
export PS1='\[\e[38;5;82m\]\u\[\e[0m\]@\[\e[38;5;69m\]\h\[\e[0m\]:\[\e[38;5;198m\]\w\[\e[0m\] \$' #w buyuk yazilirsa sadece icindeki dosya ismi görünür
- Create a `script` file named `if-statement.sh`.

```bash
#!/bin/bash
read -p "Input a number: " number
#gt greder then buyukse
#ge deseydik buyuk veya esitse
if [[ $number -gt 50 ]] #[[]] cift köseli parantez test et anlaminda,tekte calismayan bazi kodlar cift te calisiyor,parantez icine kosul yazioz
then #üstteki kosul saglandiktan sonra asagi gecmeden önce then zorunlu
  echo "The number is big."
fi 
```
man test #[[]] bunun komutlarini görürsün
...
| -lt   | less than              | #kücükse
| -ge   | greater than or equal  | #buyuk esit
| -le   | less than or equal     | #kücük esit
...
| Operator | Description |
| -------- | ----------- |
| -d file   | directory  | #bu dosya klasor mu
| -e file   | exists     | # var mi
| -f file   | ordinary file     | #düz dosyaysa
| -r file   | readable          | #okunabilir
| -s file   | size is > 0 bytes | #doluysa dosya
| -w file   | writable          | #yazdirilabilir
| -x FILE   | executable        | 
...
```bash # eger if icinde bi if daha varsa onuda fi ile kapatmalisin
#!/bin/bash
...
case $operation in
  ...
  ;;
  *)
     echo "Wrong choice..1,2,3,4 ten birini sec.." 
  ;;
esac
```
