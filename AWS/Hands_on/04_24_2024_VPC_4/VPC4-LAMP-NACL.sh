...
ssh ec2-user@10.7.5.107 (Private IP of Database Instance) # Don't forget to change the IP with your instance IP.
...
sudo dnf update -y   #nat getwayi onceden kaldirmadiysak bu iki komutu kullanip mariadbdatabase i yukleyebiliriz
sudo dnf install mariadb105-server -y

	
# 13. Create NAT instance in "Public Subnet 1a" (Other public subnets will also work). #natgateway kurduk 
...
CREATE USER admin IDENTIFIED BY '123456789'; #admin isimli kullanici olusturdu

# 22. Grant permissions to the user "admin" for database "clarusdb".
GRANT ALL ON clarusdb.* TO admin IDENTIFIED BY '123456789' WITH GRANT OPTION;  #o kullaniciya tüm yetkiler verildi

# 23. Update privileges.
FLUSH PRIVILEGES; #refresh yapip hersey calissin dedik
...
EXIT;
###
sudo hostnamectl set-hostname wordpress-server # bu iki komut ile ip yerine isim görünüyor
bash 
